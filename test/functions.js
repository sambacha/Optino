...
function printBalances() {
  ...
  console.log(
    "RESULT:  # Account                                             EtherBalanceChange               " +
      padLeft(_symbols[0], 16) +
      "               " +
      padLeft(_symbols[1], 16) +
      " Name"
  );
  console.log(
    "RESULT:                                                                                         " +
      padLeft(_symbols[2], 16) +
      "               " +
      padLeft(_symbols[3], 16)
  );
  console.log(
    "RESULT: -- ------------------------------------------ --------------------------- ------------------------------ ------------------------------ ---------------------------"
  );
  accounts.forEach(function (e) {
    ...
    var etherBalance = web3.fromWei(
      eth.getBalance(e).minus(etherBalanceBaseBlock),
      "ether"
    );
    var tokenBalances = [];
    for (j = 0; j < 4; j++) {
      tokenBalances[j] =
        _tokens[j] == null
          ? new BigNumber(0)
          : _tokens[j].balanceOf.call(e).shift(-_decimals[j]);
    ...
    console.log(
      "RESULT: " +
        ...
        pad(etherBalance) +
        " " +
        padToken(tokenBalances[0], _decimals[0]) +
        " " +
        padToken(tokenBalances[1], _decimals[1]) +
        " " +
        accountNames[e]
    );
    console.log(
      "RESULT:                                                                           " +
        padToken(tokenBalances[2], _decimals[2]) +
        " " +
        padToken(tokenBalances[3], _decimals[3])
    );
    i++;
  });
  console.log(
  ...
  console.log(
    "RESULT:                                                                           " +
      padToken(totalTokenBalances[0], _decimals[0]) +
      " " +
      padToken(totalTokenBalances[1], _decimals[1]) +
      " Total Token Balances"
  );
  console.log(
    "RESULT:                                                                           " +
      padToken(totalTokenBalances[2], _decimals[2]) +
      " " +
      padToken(totalTokenBalances[3], _decimals[3])
  );
  console.log(
    "RESULT: -- ------------------------------------------ --------------------------- ------------------------------ ------------------------------ ---------------------------"
  );
...
function printTxData(name, txId) {
  ...
  console.log(
    "RESULT: " +
      ...
      " status=" +
      txReceipt.status +
      (txReceipt.status == 0 ? " Failure" : " Success") +
      " gas=" +
      tx.gas +
      " gasUsed=" +
      txReceipt.gasUsed +
  ...
  if (txReceipt.status == 0) {
    ...
    for (var i = memory.length - 10; i < memory.length; i++) {
      console.log(
        "RESULT: debug.traceTransaction().trace.structLogs[" +
          (trace.structLogs.length - 1) +
          "].memory[" +
          i +
          "]" +
          memory[i] +
          " => '" +
          web3.toAscii(memory[i]) +
          "'"
      );
    }
  }
...
function assertEtherBalance(account, expectedBalance) {
  ...
  if (etherBalance == expectedBalance) {
    console.log(
      "RESULT: OK " + account + " has expected balance " + expectedBalance
    );
  } else {
    console.log(
      "RESULT: FAILURE " +
...
function pause(message, addSeconds) {
  var time = new Date(
    (parseInt(new Date().getTime() / 1000) + addSeconds) * 1000
  );
  console.log(
    "RESULT: Pausing '" +
      message +
...
function shiftBigNumberArray(data, decimals) {
  ...
  data.forEach(function (d) {
    results.push(d.shift(decimals).toString());
  });
...
function printTokenContractDetails(j) {
  ...
  console.log(
    "RESULT: token" +
      j +
      "ContractAddress=" +
      getShortAddressName(_tokenContractAddresses[j])
  );
  if (_tokenContractAddresses[j] != null) {
    ...
    try {
      console.log(
        "RESULT: token" +
          j +
          ".owner/new=" +
          getShortAddressName(contract.owner.call()) +
          "/" +
          getShortAddressName(contract.newOwner.call())
      );
    } catch (error) {
    ...
    console.log(
      "RESULT: token" +
        j +
        ".totalSupply=" +
        contract.totalSupply.call().shift(-decimals)
    );
    try {
      console.log(
        "RESULT: token" +
          j +
          ".strike=" +
          contract.strike.call().shift(-decimals)
      );
      console.log(
        "RESULT: token" + j + ".bound=" + contract.bound.call().shift(-decimals)
      );
    } catch (e) {}
    ...
    if (j > 0) {
      var ownershipTransferredEvents = contract.OwnershipTransferred(
        {},
        { fromBlock: tokenFromBlock[j], toBlock: latestBlock }
      );
      i = 0;
    ...
    var approvalEvents = contract.Approval(
      {},
      { fromBlock: tokenFromBlock[j], toBlock: latestBlock }
    );
    i = 0;
    approvalEvents.watch(function (error, result) {
      ...
      console.log(
        "RESULT: token" +
          ...
          result.blockNumber +
          " tokenOwner=" +
          getShortAddressName(result.args.tokenOwner) +
          " spender=" +
          getShortAddressName(result.args.spender) +
          " tokens=" +
          result.args.tokens.shift(-decimals)
      );
    });
    approvalEvents.stopWatching();

    var transferEvents = contract.Transfer(
      {},
      { fromBlock: tokenFromBlock[j], toBlock: latestBlock }
    );
    i = 0;
    transferEvents.watch(function (error, result) {
      ...
      console.log(
        "RESULT: token" +
          ...
          result.blockNumber +
          " from=" +
          getShortAddressName(result.args.from) +
          " to=" +
          getShortAddressName(result.args.to) +
          " tokens=" +
          result.args.tokens.shift(-decimals)
      );
    });
...
function getTokenContractDeployed() {
  ...
  if (_factoryContractAddress != null && _factoryContractAbi != null) {
    var contract = eth
      .contract(_factoryContractAbi)
      .at(_factoryContractAddress);

    var latestBlock = eth.blockNumber;
    var i;

    var tokenDeployedEvents = contract.TokenDeployed(
      {},
      { fromBlock: _factoryFromBlock, toBlock: latestBlock }
    );
    i = 0;
    tokenDeployedEvents.watch(function (error, result) {
      console.log(
        "RESULT: get TokenDeployed " +
          i++ +
          " #" +
          result.blockNumber +
          " " +
...
function printPriceFeedContractDetails() {
  console.log(
    "RESULT: priceFeedContractAddress=" +
      getShortAddressName(_priceFeedContractAddress)
  );
  ...
  if (_priceFeedContractAddress != null && _priceFeedContractAbi != null) {
    var contract = web3.eth
      .contract(_priceFeedContractAbi)
      .at(_priceFeedContractAddress);
    ...
    console.log(
      "RESULT: priceFeed.value=" + contract.value.call().shift(-18) + " ETH/USD"
    );
    ...
    var setValueEvents = contract.SetValue(
      {},
      { fromBlock: _priceFeedFromBlock, toBlock: latestBlock }
    );
    i = 0;
    setValueEvents.watch(function (error, result) {
      console.log(
        "RESULT: SetValue " +
          ...
          result.blockNumber +
          " value=" +
          result.args.value.shift(-18) +
          " hasValue=" +
          result.args.hasValue
      );
    });
...
function printPriceFeedAdaptorContractDetails() {
  console.log(
    "RESULT: priceFeedAdaptorContractAddress=" +
      getShortAddressName(_priceFeedAdaptorContractAddress)
  );
  if (
    _priceFeedAdaptorContractAddress != null &&
    _priceFeedAdaptorContractAbi != null
  ) {
    var contract = web3.eth
      .contract(_priceFeedAdaptorContractAbi)
      .at(_priceFeedAdaptorContractAddress);
    console.log(
      "RESULT: priceFeedAdaptor.sourceAddress=" + contract.sourceAddress.call()
    );
...
function getOptinoTokens() {
  ...
  if (
    _optinoFactoryContractAddress != null &&
    _optinoFactoryContractAbi != null
  ) {
    var contract = web3.eth
      .contract(_optinoFactoryContractAbi)
      .at(_optinoFactoryContractAddress);
    var latestBlock = eth.blockNumber;
    var seriesAddedEvents = contract.SeriesAdded(
      {},
      { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
    );
    var i = 0;
    seriesAddedEvents.watch(function (error, result) {
      console.log(
        "RESULT: got SeriesAdded " +
          i++ +
          " #" +
          result.blockNumber +
          " " +
...
function printOptinoFactoryContractDetails() {
  console.log(
    "RESULT: optinoFactoryContractAddress=" +
      getShortAddressName(_optinoFactoryContractAddress)
  );
  ...
  if (
    ...
    // console.log("RESULT: contract=" + JSON.stringify(contract));
    console.log(
      "RESULT: optinoFactory.owner/new=" +
        getShortAddressName(contract.owner.call()) +
        "/" +
        getShortAddressName(contract.newOwner.call())
    );
    console.log(
      "RESULT: optinoFactory.optinoTokenTemplate=" +
        getShortAddressName(contract.optinoTokenTemplate.call())
    );
    console.log(
      "RESULT: optinoFactory.message/fee='" +
        contract.message.call() +
        "'/" +
        contract.fee.call().shift(-16) +
        "%"
    );
    ...
    for (var feedIndex = 0; feedIndex < feedLength; feedIndex++) {
      ...
      console.log(
        "RESULT: optinoFactory.getFeedByIndex(" +
          feedIndex +
          "). " +
          JSON.stringify(tokenDecimals)
      );
    }
...
<<<<<<< HEAD
    for (var seriesIndex = 0; seriesIndex < seriesLength; seriesIndex++) {
      ...
      console.log(
        "RESULT: optinoFactory.getSeriesByIndex(" +
          seriesIndex +
          ")=" +
      ...
      console.log(
        "RESULT:   optinoFactory.getSeriesByIndex(" +
          ...
          expiry +
          ", strike=" +
          strike.shift(-feedDecimals0) +
          ", bound=" +
          bound.shift(-feedDecimals0) +
          ", spot=" +
          spot.shift(-feedDecimals0) +
          ", optinoToken=" +
          optinoToken +
          ", coverToken=" +
          coverToken +
      ...
      [optinoToken, coverToken].forEach(function (tokenAddress) {
        console.log(
          "RESULT:     token: " +
            getShortAddressName(tokenAddress) +
            " on " +
            getShortAddressName(pair[0]) +
            "/" +
            getShortAddressName(pair[1])
        );
        var tokenContract = web3.eth
          .contract(_optinoTokenContractAbi)
          .at(tokenAddress);
        var tokenDecimals = tokenContract.decimals.call();
        var collateralToken = tokenContract.collateralToken.call();
        var collateralTokenContract = web3.eth
          .contract(_optinoTokenContractAbi)
          .at(collateralToken);
        ...
        var spots = [
          new BigNumber("9769.26390498279639").shift(rateDecimals),
          new BigNumber(50).shift(rateDecimals),
          new BigNumber(100).shift(rateDecimals),
          new BigNumber(150).shift(rateDecimals),
          new BigNumber(200).shift(rateDecimals),
          new BigNumber(250).shift(rateDecimals),
          new BigNumber(300).shift(rateDecimals),
          new BigNumber(350).shift(rateDecimals),
          new BigNumber(400).shift(rateDecimals),
          new BigNumber(450).shift(rateDecimals),
          new BigNumber(500).shift(rateDecimals),
          new BigNumber(1000).shift(rateDecimals),
          new BigNumber(10000).shift(rateDecimals),
          new BigNumber(100000).shift(rateDecimals),
        ];
        console.log(
          "RESULT:       .owner/new=" +
            getShortAddressName(tokenContract.owner.call()) +
            "/" +
            getShortAddressName(tokenContract.newOwner.call())
        );
        console.log(
          "RESULT:       .details='" +
        ...
        console.log(
          "RESULT:       .totalSupply=" +
            tokenContract.totalSupply.call().shift(-tokenDecimals)
        );
        console.log(
          "RESULT:       .seriesKey=" + tokenContract.seriesKey.call()
        );
        console.log(
          "RESULT:       .isCover/optinoPair=" +
            tokenContract.isCover.call() +
            "/" +
            getShortAddressName(tokenContract.optinoPair.call())
        );
        console.log(
          "RESULT:       .collateralToken/decimals/rateDecimals: " +
            getShortAddressName(collateralToken) +
            "/" +
            collateralDecimals +
            "/" +
            rateDecimals
        ...
        console.log(
          "RESULT:       .closedOrSettled=" +
            tokenContract.balanceOf.call(NULLACCOUNT).shift(-collateralDecimals)
        );
        ...
        console.log(
          "RESULT:       .currentSpot/currentSpotAndPayoff(1)=" +
            tokenContract.currentSpot.call().shift(-rateDecimals) +
            "/" +
            tokenContract.currentSpotAndPayoff
              .call(oneToken)[1]
              .shift(-collateralDecimals)
        );
        console.log(
          "RESULT:       .spot/spotAndPayoff(1)=" +
            tokenContract.spot.call().shift(-rateDecimals) +
            "/" +
            tokenContract.spotAndPayoff
              .call(oneToken)[1]
              .shift(-collateralDecimals)
        );
        console.log(
          "RESULT:       .payoffForSpot(1, " +
            spots[4].shift(-rateDecimals) +
            ")=" +
            tokenContract.payoffForSpot
              .call(oneToken, spots[4])
              .shift(-collateralDecimals)
        );
        console.log(
          "RESULT:       .payoffForSpots(1, " +
            JSON.stringify(shiftBigNumberArray(spots, -rateDecimals)) +
            ")=" +
            JSON.stringify(
              shiftBigNumberArray(
                tokenContract.payoffForSpots.call(oneToken, spots),
                -collateralDecimals
              )
            )
        );
        console.log(
          "RESULT:       .getInfo=" +
            JSON.stringify(tokenContract.getInfo.call())
        );
        console.log("RESULT:       .getFeedInfo=" + JSON.stringify(feedInfo));
        console.log(
          "RESULT:       .getPricingInfo=" +
            JSON.stringify(tokenContract.getPricingInfo.call())
        );

        var optinoTransferEvents = tokenContract.Transfer(
          {},
          { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
        );
        var j = 0;
        optinoTransferEvents.watch(function (error, result) {
          console.log(
            "RESULT:     .Transfer " +
              ...
              getShortAddressName(result.args.to) +
              " tokens=" +
              result.args.tokens.shift(-optinoTokenDecimals)
          );
        });
        optinoTransferEvents.stopWatching();

        var optinoTokenCloseEvents = tokenContract.Close(
          {},
          { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
        );
        j = 0;
        optinoTokenCloseEvents.watch(function (error, result) {
          console.log(
            "RESULT:       .Close " +
              ...
              result.blockNumber +
              " optinoToken=" +
              getShortAddressName(result.args.optinoToken) +
              " coverToken=" +
              getShortAddressName(result.args.coverToken) +
              " tokenOwner=" +
              getShortAddressName(result.args.tokenOwner) +
              " tokens=" +
              result.args.tokens.shift(-tokenDecimals) +
              " collateralRefunded=" +
              result.args.collateralRefunded.shift(-collateralDecimals)
          );
=======
      for (var seriesIndex = 0; seriesIndex < seriesLength; seriesIndex++) {
        var series = contract.getSeriesByIndex.call(seriesIndex);
...
>>>>>>> ogDAO-master
        ...
        optinoTokenCloseEvents.stopWatching();
        var optinoTokenPayoffEvents = tokenContract.Payoff(
          {},
          { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
        );
        j = 0;
        optinoTokenPayoffEvents.watch(function (error, result) {
          console.log(
            "RESULT:       .Payoff " +
              ...
              result.blockNumber +
              " optinoOrCoverToken=" +
              getShortAddressName(result.args.optinoOrCoverToken) +
              " tokenOwner=" +
              getShortAddressName(result.args.tokenOwner) +
              " tokens=" +
              result.args.tokens.shift(-tokenDecimals) +
              " collateralPaid=" +
              result.args.collateralPaid.shift(-collateralDecimals)
          );
        });
        optinoTokenPayoffEvents.stopWatching();
        var optinoTokenLogInfoEvents = tokenContract.LogInfo(
          {},
          { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
        );
        j = 0;
        optinoTokenLogInfoEvents.watch(function (error, result) {
          console.log(
            "RESULT:       .LogInfo " +
              ...
              result.args.note +
              " addr=" +
              getShortAddressName(result.args.addr) +
              " number=" +
              result.args.number
          );
        });
    ...
    var messageUpdatedEvents = contract.MessageUpdated(
      {},
      { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
    );
    i = 0;
    ...
    var feeUpdatedEvents = contract.FeeUpdated(
      {},
      { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
    );
    i = 0;
    ...
    var tokenDecimalsUpdatedEvents = contract.TokenDecimalsUpdated(
      {},
      { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
    );
    i = 0;
    ...
    var seriesSpotUpdatedEvents = contract.SeriesSpotUpdated(
      {},
      { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
    );
    i = 0;
    ...
    var optinosMintedEvents = contract.OptinosMinted(
      {},
      { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
    );
    i = 0;
    optinosMintedEvents.watch(function (error, result) {
      ...
      var optinoTokenContract = web3.eth
        .contract(_optinoTokenContractAbi)
        .at(result.args.optinos[0]);
      var optinoDecimals = optinoTokenContract.decimals.call();
      var collateralTokenContract = web3.eth
        .contract(_optinoTokenContractAbi)
        .at(optinoTokenContract.collateralToken.call());
      var collateralDecimals = collateralTokenContract.decimals.call();
      console.log(
        "RESULT: OptinosMinted " +
          j++ +
          " #" +
          result.blockNumber +
          " seriesKey=" +
          result.args.seriesKey +
          " optinoToken=" +
          getShortAddressName(result.args.optinos[0]) +
          " coverToken=" +
          getShortAddressName(result.args.optinos[1]) +
          " tokens=" +
          result.args.tokens.shift(-optinoDecimals) +
          " ownerFee=" +
          result.args.ownerFee.shift(-collateralDecimals) +
          " integratorFee=" +
          result.args.integratorFee.shift(-collateralDecimals)
      );
    });
    optinosMintedEvents.stopWatching();

<<<<<<< HEAD
    var optinoTokenLogInfoEvents = contract.LogInfo(
      {},
      { fromBlock: _optinoFactoryFromBlock, toBlock: latestBlock }
    );
    j = 0;
